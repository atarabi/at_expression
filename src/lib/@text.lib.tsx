({
    load(force: boolean = false): Atarabi.Text.Lib {
        ...
        const MSG = {
            ...
            UNKNOWN_KEY: (key: string, context: string = "attribute") =>
                `Unknown ${context}: "${key}"`,
            INVALID_OPTION: (key: string, options: (string | number)[], actual: any) =>
                `"${key}" must be one of [${options.join(", ")}], but got: "${actual}"`,
            RANGE_ERROR: (key: string, min: number, max: number, actual: any) =>
                `"${key}" must be between ${min} and ${max}, but got: "${actual}"`,
            REQUIRED: (key: string) =>
                `"${key}" is required and cannot be empty.`,
            PRECONDITION_REQUIRED: (action: string, precondition: string) =>
                `"${precondition}" must be called before calling "${action}"`,
        } as const;
        ...
        function createMatcher(matchers?: CharMatchers): (g: string) => boolean {
            ...
            const fns = items.map(m => {
                if (typeof m === "string") {
                    ...
                    if (!cc) throw new Error(MSG.UNKNOWN_KEY(m, "CharClass"));
        ...
        function applyTextStyleUniversal(style: TextStyleProperty, field: string, value: any, startIndex?: number, count?: number): TextStyleProperty {
            ...
            if (!methodName) throw new Error(MSG.UNKNOWN_KEY(field, "style"));
            ...
            if (typeof fn !== "function") throw new Error(MSG.UNKNOWN_KEY(methodName, "style function"));
        ...
        type StylesWithOps = { pre: boolean; text?: string; styles: RangeWithStyle[]; ops: TransformOp[]; };

        interface TextStyleResolver {
            resolve(text: string): RangeWithStyle[] | StylesWithOps;
        }
        ...
        abstract class TextStyleBuilder<Rule> implements TextStyleResolver, Atarabi.Text.TextStyleBuilder<Rule> {
            ...
            abstract resolve(text: string): RangeWithStyle[] | StylesWithOps;
        }
        ...
        function toRegExp(m: CharMatcher): RegExp | RegExp[] {
            if (typeof m === "string") {
                ...
                if (!r) throw new Error(MSG.UNKNOWN_KEY(m, "CharClass"));
                return r.re;
        ...
        // replace
        type ReplaceTransformKey = Atarabi.Text.ReplaceTransformKey;
        type ReplaceTransformMap = Atarabi.Text.ReplaceTransformMap;

        function prepareReplaceArgs(pattern: RegExp | string, replacement: string | ((text: string) => string), transforms: ReplaceTransformMap = {}): {
            pattern: RegExp;
            replacement: string;
            transforms: ReplaceTransformMap
        } {
            ...
            const finalTransforms: ReplaceTransformMap = { ...transforms };
        ...
        function getGroupOffsets(match: any): Record<number | string, { start: number; end: number }> {
            const offsets: Record<number | string, { start: number; end: number }> = {};

            match.indices.forEach((range: [number, number] | undefined, i: number) => {
                if (range) {
                    offsets[i] = {
        ...
        function applyReplace(input: string, pattern: RegExp, replacement: string, transforms: ReplaceTransformMap): { output: string; ops: TransformOp[] } {
            ...
            while ((match = re.exec(input)) !== null) {
                ...
                for (const part of parts) {
                    if (part.type === "text") {
                        ...
                        if (offset) {
                            const transformKey: ReplaceTransformKey = typeof idx === 'number' ? `$${idx}` : `$<${idx}>`;
                            const transformFn = transforms[transformKey];

                            const moveOp: TransformOp = {
                                type: "move",
                ...
                if (fragments.length > 0) {
                    ...
                    for (let i = 1; i < fragments.length; i++) {
                        ...
                        if (isContinuous) {
                            ...
                            if (finalized.count > 0) newRanges.push(finalized);
                            merged = { ...next };
                        }
                    }
                    const finalized = finalizeRange(merged);
        ...
        function finalizeRange(range: RangeWithStyle): RangeWithStyle {
        ...
        class AsMarkdown implements TextStyleResolver {
            private readonly pattern = /(?<h>^(?<h_level>#{1,6})\s+(?<h_t>.*)$)|(?<b>\*\*(?<b_t>.*?)\*\*)|(?<i>(?<i_marker>[*_])(?<i_t>.*?)\k<i_marker>)|(?<link>\[(?<l_t>.*?)\]\((?<l_u>.*?)\))/gdm;
            ...
            private parse(input: string, offset: number = 0): { styles: RangeWithStyle[]; ops: TransformOp[]; length: number } {
                ...
                while ((match = this.pattern.exec(input)) !== null) {
                    ...
                    let content = "", contentStartInInput = 0, currentStyle: any = undefined;

                    if (groups.h) {
                        content = groups.h_t; contentStartInInput = indices.h_t[0];
                        currentStyle = this.style[`h${groups.h_level.length}` as keyof Atarabi.Text.MarkdownStyle];
                    } else if (groups.b) {
                        content = groups.b_t; contentStartInInput = indices.b_t[0];
                        currentStyle = this.style.b;
                    } else if (groups.i) {
                        content = groups.i_t; contentStartInInput = indices.i_t[0];
                        currentStyle = this.style.i;
                    } else if (groups.link) {
                        content = groups.l_t; contentStartInInput = indices.l_t[0];
            ...
            constructor(protected style: Atarabi.Text.CustomMarkupStyle) {
                ...
                if (this.delimiters.length > 0) {
                    ...
                    this.pattern = new RegExp(
                        `(?<!\\\\)(?:\\\\\\\\)*(?<delim>${escapedDelims})(?<content>(?:\\\\.|(?!\\k<delim>).)*?)(?<!\\\\)(?:\\\\\\\\)*\\k<delim>`,
                        'gdm'
                    );
                }
            }
        ...
        class AsTagMarkup implements TextStyleResolver {
            constructor(protected style: Atarabi.Text.TagMarkupStyle, protected variables: Atarabi.Text.TagMarkupVariables) { }
            resolve(text: string): StylesWithOps {
                const { styles, ops } = this.parse(text);
                return { pre: false, styles, ops };
            }
            private readonly pattern = /(?<!\\)<(?<tagname>[^>\s/]+)(?<attrs>.*?)>(?<content>.*?)<\/\k<tagname>>/gdm;
            private readonly stylePattern = /style\s*=\s*(?<quote>["'”])(?<sValue>.*?)\k<quote>/d;
            private parse(input: string, offset: number = 0): { styles: RangeWithStyle[]; ops: TransformOp[]; length: number } {
                const ops: TransformOp[] = [];
                const ranges: RangeWithStyle[] = [];
                let currentOutputPos = 0;
                    ...
                    if (groups.attrs && groups.attrs.includes("style=")) {
                        ...
                        if (styleMatch) {
                            const inline = Object.fromEntries(
                                styleMatch.groups!.sValue.split(';').reduce((acc: [string, any][], s: string) => {
            ...
            private parseInlineStyle(key: string, value: string): boolean | string | number | number[] {
                ...
                switch (key) {
                    ...
                    case "verticalScaling":
                        ...
                        if (isNaN(v)) throw new Error(MSG.TYPE_MISMATCH(key, EXPECTED.NUMBER, value));
                        return v;
                    case "fillColor":
                    case "strokeColor":
                        try {
        ...
        class TextStyleContext implements Atarabi.Text.TextInputter, Atarabi.Text.TextTransformer, Atarabi.Text.TextStyleFacade {
            ...
            input(text: string): this {
                if (this.items.length) throw new Error(MSG.PRECONDITION_REQUIRED("input()", "as...(), by...(), forEach...()"));
                if (this.transforms.length) throw new Error(MSG.PRECONDITION_REQUIRED("input()", "transform()"));
                this.inputText = text;
                return this;
            }
            transform(fn: (text: string, ctx: TextTransformContext) => string): this {
                if (this.items.length) throw new Error(MSG.PRECONDITION_REQUIRED("Mtransform()", "as...(), by...(), forEach...()"));
                this.transforms.push(fn);
                return this;
            }
            replace(pattern: RegExp | string, replacement: string | ((text: string) => string), transforms: ReplaceTransformMap = {}): this {
