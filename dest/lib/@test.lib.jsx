({
    load() {
        function assertNoCycle(value, seen, path) {
        ...
        function deepEqualImpl(a, b, path) {
            if (a === b)
                return true;
            ...
            if (Array.isArray(a)) {
                ...
                for (let i = 0; i < a.length; i++) {
                    if (!deepEqualImpl(a[i], b[i], `${path}[${i}]`))
                        return false;
                }
                return true;
            }
            ...
            for (const k of ka) {
                if (!(k in b))
                    return false;
                if (!deepEqualImpl(a[k], b[k], `${path}.${k}`))
                    return false;
            }
            return true;
        }
        function nearImpl(a, b, eps, path) {
